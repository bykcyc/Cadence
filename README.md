...
3. On the **first transcription**, Cadence automatically sets up its local engine — it installs the
   ...
   and downloads the speech model (cached afterwards). No console, no manual steps. Transcription uses
   the **ONNX Parakeet** engine in one of two modes (*Settings → Recording*): **CPU** — the lightweight
   default (~1 GB, no GPU); or **GPU** — onnxruntime-gpu for ~7× faster transcription of long meetings
   (downloads the CUDA libraries, ~1.8 GB, the first time you switch). Speaker diarization uses a
   separate NeMo/PyTorch engine (set up only if you use it).
